...
import { useStore, useLiveTasks } from "@/lib/store";
...
export default function TasksView() {
  ...
  const toggleDone = (id: string, done: boolean) => dbUpdate("tasks", id, { status: done ? "done" : "open" });
  ...
  return (
    <>
      <div className="pagehead">
        ...
        <button className="btn primary" onClick={() => openDrawer("task")}>+ New task</button>
      </div>
      <DataTable rows={tasks} columns={columns} rowKey={(t) => t.id}
        onRowClick={(t) => openDrawer("task", t.id)}
